        ...
        if uploaded_file:
            ...
            if st.session_state['uploaded_file_hash'] != current_hash:
                ...
                if chunks:
                    ...
                    st.session_state['financial_metrics_data'] = extract_financial_metrics(st.session_state['text_chunks'])
                    
                    st.success(f"Analysis Complete! A knowledge base of {len(chunks)} chunks has been created.")
                    st.info(f"You can now ask questions about '{uploaded_file.name}' in the 'Chat with Data' tab and view metrics in the 'Financial Metrics' tab.")
                else:
    ...
    with main_tabs[1]:
        ...
        if not st.session_state.get('financial_metrics_data'):
            ...
            if isinstance(metrics_data, dict) and any(metrics_data.values()):
                        ...
                        if metrics:
                            ...
                            for metric_name, data in metrics.items():
                                with cols[col_index]:
                                    ...
                                    delta_color_value = data.get("delta_color", "off") if delta_value is not None else "off"
                ...
                st.info("No financial metrics were found in the document. You can now chat with the PDF to get your answers.")
